...
void sfd4(double *H, double *dH, double r, double s)
{
  ...
  const double h3r = 0.25 * (1 + s);
  const double h4r = -0.25 * (1 + s);
  /***********************************************/
  const double h1s = -0.25 * (1 - r);
  const double h2s = -0.25 * (1 + r);
  const double h3s = 0.25 * (1 + r);
      ...
      for (int pdf = 0; pdf < npd; ++pdf)
      {
        for (int sdf = 0; sdf < nsd; ++sdf)
        {
          for (int j = 0; j < nsn; ++j)
          {
            ...
            dXm[npd * sdf + pdf] += dHm[pdf * nsn + j] * Xm[sdf * nsn + j];
            dxm[npd * sdf + pdf] += dHm[pdf * nsn + j] * (Xm[sdf * nsn + j] + Um[sdf * nsn + j]);
          }
        }
      }
        ...
        { // loop over cols
          ...
          { // loop over rows
            ...
            if (fabs(C_Ns[k] * C_Ns[j]) > 1e-50)
            {
              ...
              (*len)++;
              if (*len >= len_guess)
                std::cout << "Error, len is too small: len = " << len_guess << std::endl;
            }
            ...
            (*len)++;
            if (*len >= len_guess) std::cout << "Error, len is too small: len = " << len_guess << std::endl;
          }
          */
...
void evaluateContactConstraints(double *GPs, int *ISN, int *IEN, int *N, double *AABBmin, double *AABBmax, int *head, int *next, double *X, int *elementID, int *segmentID, int n, int nsn, int nsd, int npd, int ngp, int nen, int nes, int neq, double longestEdge)
{
  ...
  if (nsn == 8 || nsn == 4)
  {
    // If the segment element is a quad (linear or quadratic) then it is subdivided to 4 triangles (with a common vertex Xc in the centre of mass):
    ntr = 4;
  }
  ...
  for (int e = 0; e < n; ++e)
  {
    ...
    for (int it = 0; it < ntr; ++it)
    {
      ...
      if (nsd == 3)
      {
        ...
        {
          // X coord of n-th node: Xm[0*nsn+n];
          // Y coord of n-th node: Xm[1*nsn+n];
          // Z coord of n-th node: Xm[2*nsn+n];
          //  (3, 7,11)---(2, 6,10)
          //     |      Xc    |
          //  (0, 4, 8)---(1, 5, 9)
          // X coords:
              ...
              if (isInside)
              {
                ...
                if (d > GPs[(nsd + 2) * n * ngp + v] && d < 25)
                {
                  ...
                  case 6:
                    ...
                    r = ((Xp[0] - Xm[0]) * (Xm[1] - Xm[0]) +
                         (Xp[1] - Xm[6]) * (Xm[7] - Xm[6]) +
                         (Xp[2] - Xm[12]) * (Xm[13] - Xm[12])) /
                        r_len;

                    s = ((Xp[0] - Xm[0]) * (Xm[2] - Xm[0]) +
                         (Xp[1] - Xm[6]) * (Xm[8] - Xm[6]) +
                         (Xp[2] - Xm[12]) * (Xm[14] - Xm[12])) /
                        s_len;
                    break;
                  case 8:
                    r_len = pow(Xm[1] - Xm[0], 2.0) +
                  ...
                  do
                  {
                    ...
                    niter++;
                  } while (dr_norm > 1e-10 && niter < max_niter);
