...
extern "C" {  // only need to export C interface if used by C++ source code
  ...
  void __declspec(dllexport) assembleContactResidualAndStiffness(double* Gc, double* vals, double* rows, double* cols, int* len, double* GPs, int* ISN, int* IEN, double* X, double* U, double* H, double* dH, double* gw, double* activeGPsOld, int neq, int nsd, int npd, int ngp, int nes, int nsn, int nen, int GPs_len, double epss, bool keyContactDetection);
